...
int main()
{
    ...
    uint16_t frame[16][16][48] = {
        {
            {0, 0x1FFC, 0, 0x1FFC, 0, 0x1FFC},
            {0x1FFC, 0, 0x1FFC, 0, 0x1FFC, 0},
            {0, 0x1FFC, 0, 0x1FFC, 0, 0x1FFC},
            {0x1FFC, 0, 0x1FFC, 0, 0x1FFC, 0},
            {0, 0x1FFC, 0, 0x1FFC, 0, 0x1FFC},
            {0x1FFC, 0, 0x1FFC, 0, 0x1FFC, 0},
            ...
            {0x1FFC, 0, 0x1FFC, 0, 0x1FFC, 0},
            {0, 0x1FFC, 0, 0x1FFC, 0, 0x1FFC},
            {0x1FFC, 0, 0x1FFC, 0, 0x1FFC, 0x1FFC},
            {0, 0x1FFC, 0, 0x1FFC, 0, 0x1FFC},
            {0x1FFC, 0, 0x1FFC, 0, 0x1FFC, 0x1FFC},
        },
        {
    ...
    // irq_set_exclusive_handler(PIO0_IRQ_0, isr_handler);
    // pio_set_irq0_source_enabled(pio, pis_interrupt0, true);
    // irq_set_enabled(PIO0_IRQ_0, true);
    
    // offset = pio_add_program(pio0, &mbi_graph_program);
    // pio_mbi_graph_program_init(pio0, 2, offset, 10000000, 22, 0);
    // pio_sm_put_blocking(pio0, 2, (512 << 16) | 0);
    // pio_sm_put_blocking(pio0, 2, (0xFFFF<< 16) | 0);
    // sleep_ms(10000);
    mbi5252_send_frame(frame, 256);
    // mbi5252_send_frame(frame, 256);

    // restart_glck(pio0, 2, offset, 10000000, 22, 0);
    // pio_sm_put_blocking(pio0, 2, (512 << 16) | 0);
    // pio_sm_put_blocking(pio0, 2, (0xFFFE<< 16) | 0);
    ...
    while (true) {
        ...
        // mbi5252_send_frame(frame, 256);
        // frame[15][15][5]+=100;
        // pio_sm_put_blocking(pio, 1, (94 << 20) | (0 << 16));
        // pio_sm_put_blocking(pio, 1, (0xAA55 << 16) | 0);
        // pio_sm_put_blocking(pio, 1, (0xAA55 << 16) | 0);
        // pio_sm_put_blocking(pio, 1, (0xAA55 << 16) | 0);
        // pio_sm_put_blocking(pio, 1, (0xAA55 << 16) | 0);
        // pio_sm_put_blocking(pio, 1, (0xAA55 << 16) | 0);
        // pio_sm_put_blocking(pio, 1, (0xAA55 << 16) | 0);
        // sleep_us(20);
        // pio_sm_put_blocking(pio, 1, (13 << 20) | (1 << 16));
        // pio_sm_put_blocking(pio, 1, (0xAA55 << 16) | 0);

        // pio_mbi_data_latch(&pio_mbi, frame[0][0], 6);
        // sleep_us(20);
        // pio_mbi_vsync(&pio_mbi);
    }
}
